function getCpuUsage(callback){
    chrome.system.cpu.getInfo(function(info){
        ...
        for(var i = 0;i<info.processors.length;i++){
            ...
            cpu_history.last_user[i] = info.processors[i].usage.suer;
...
function updateMemHistory(){
    getMemUsage(function(usage){
        ...
        mem_history.used.push(Math.round(usage.capacity - usage.availableCapcity)/usage.capacity*100)
        showMem();
    });
}
...
function showCpu(){
    var history = {
        lables: (function(){
            for(var i = 0,lables = [];i < ponits_num;lables.push(''),i++);
            return lables
        }),
        datesets:[
            {
...
function showMem(){
    var history = {
        lables:(function(){for(var i = 0,lables = [];i<ponits_num;lables.push(''),i++);return lables})(),
        datesets:[
            {
                fillColor:"rgba(220,220,220,0.5)",
    ...
    var now_ctx = document.getElementById("mem_toatal").getContext("2d");
...
function init_cpu_history(){
    ...
    chrome.system.cpu.getInfo(function(info){
        for(var i = 0;i<info.processors.length;i++){
            cpu_history.last_total.push(info.processors[i].usage.tatal);
